...
from model.room_model import Kitchen, LivingRoom, Bedroom
...
# This a function table, mapping from the room.type to the corresponding constructor
ROOM_TYPE_MAPPING = {
...
# This is a function table mapping device types to their corresponding constructors
DEVICE_TYPE_MAPPING = {
...
class MainModel:
                        ...
                        for tab_data in data:
                            ...
                            if tab_type in ROOM_TYPE_MAPPING:
                                ...
                                for device_data in room_data.get("devices", []):
                                    ...
                                    #     room.devices.append(elec_app.Television(device_id=device_id))
                                    # elif device_type == "Lamp":
                                    #     room.devices.append(elec_app.Lamp(device_id=device_id))
                                    # elif device_type == "Bedside Lamp":
                                    #     room.devices.append(elec_app.BedsideLamp(device_id=device_id))
                                    # elif device_type == "Refrigerator":
                                    #     room.devices.append(elec_app.Refrigerator(device_id=device_id))
                                    # elif device_type == "Electric Kettle":
                                    #     room.devices.append(elec_app.ElectricKettle(device_id=device_id))
                                    # elif device_type == "Air Conditioner":
                                    #     room.devices.append(elec_app.AirConditioner(device_id=device_id))
    ...
    #     same_type_number = sum(1 for tab in self.tabs if tab.tab_type == tab_type) + 1  # Start with 1

    #     if tab_type == "Welcome":
            
    #         # Create a Welcome tab
    #         new_tab = WelcomeModel(tab_type, same_type_number, config.WELCOME_MESSAGE)
    #     elif tab_type in ROOM_TYPE_MAPPING:
            
    #         # Create a RoomTabModel for specific room types
    #         room_class = ROOM_TYPE_MAPPING[tab_type]
    #         new_room = room_class(room_id=same_type_number)  # Example: Use same_type_number as room_id
    #         new_tab = RoomTabModel(tab_type, same_type_number, new_room)
    #     else:
            
    #         # Default to a generic TabModel
    #         new_tab = TabModel(tab_type, same_type_number)
    ...
    def add_device(self, tab_to_be_added_on_name, device_type):
        ...
        Parameters:
        tab_to_be_added_on_name (str): The name of the tab where the device will be added.
        device_type (str): The type of the device to add.
        Returns:
    ...
    def remove_current_device(self, tab_full_name, device_full_name):
        ...
        Parameters:
        tab_full_name (str): The name of the tab where the device will be remove.
        device_full_namee (str): The full name of the device to be removed.
        Returns:
        tuple: (success (bool), message (str))
        """
        try:
